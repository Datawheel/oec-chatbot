...
def get_api(query, TABLES_PATH, step=None, **kwargs):
    ...
    if step == 'request_tables_to_lm_from_db':
        ...
        return get_api(query, TABLES_PATH, step='get_api_params_from_lm', **{'table': table})
        ...
        print("API:", api_url)
        return get_api(query, TABLES_PATH, step='fetch_data', **{**kwargs, **{'api': api}})

    elif step == 'fetch_data': 
        api = kwargs['api']
        data, df, response = api.fetch_data()
        return get_api(query, TABLES_PATH, step='agent_answer', **{**kwargs, **{'df': df, 'response': response, 'api_url': api_url}})
